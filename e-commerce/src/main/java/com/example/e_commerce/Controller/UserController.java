...
//	}
	...
	@PutMapping("/updateuser/{userId}")
	public ResponseEntity<UserResponseEntity> uploadUser(
	    ...
	    if (userRequestEntity.getUsername() != null && !userRequestEntity.getUsername().isEmpty()) {
	        existingUser.setUsername(userRequestEntity.getUsername());
	    }

	    if (userRequestEntity.getUserType() != null) {
	        existingUser.setUserType(userRequestEntity.getUserType());
	    }

	    if (userRequestEntity.getEmail() != null && !userRequestEntity.getEmail().isEmpty()) {
	        existingUser.setEmail(userRequestEntity.getEmail());
	    }
	    ...
	    if (userRequestEntity.getRole() != null && !userRequestEntity.getRole().isEmpty()) {
	        existingUser.setRole(userRequestEntity.getRole());
	    }

	    if (userRequestEntity.getUserStatus() != null) {
	        existingUser.setUserStatus(userRequestEntity.getUserStatus());
	    }
...
//				System.out.println("user Type:"+userResponseEntity);
	...
	public ResponseEntity<UserResponseEntity> deleteUser(@PathVariable("id") Integer id) {
	    ...
	    userService.delete(user.getId()); // Delete user regardless of status

	    response.setStatusCode(200);
	    response.setMessage("User with ID " + user.getId() + " deleted successfully");
	    return new ResponseEntity<>(response, HttpStatus.OK);
	}
